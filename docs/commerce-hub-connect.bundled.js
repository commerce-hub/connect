        ...
        <div class="required-info-flow">
          ...
          <div class="required-info-documents">
            ...
            <div class="primitive-stack">
              ${t.map((e,t)=>{const r=Gj(e,t),i=Jj(e)?this.requiredInfoValues[r]:this.requiredInfoFileNames[r];return Jj(e)?eE({doc:e,key:r,value:i??""}):Ry`
                  <div
...
`)({label:"Required document",requirements:rE(e),actionLabel:"Choose file",fileName:i,action:"open-required-info-file-picker",disabled:n})}
                  </div>
                `})}
            </div>
          </div>
      ...
      `})}renderNoSavedPaymentMethods(){const e=Qx(this.designConfig),t="delete-payment-source"===this.pendingAction,r=cj(this.interactionResponse),n=Ej(this.interactionResponse,"ADD_ACCOUNT"),i=r.bankAccount&&Aj(this.interactionResponse);return this.renderWidgetShell({title:e.title,subtitle:e.subtitle,showClose:!1,showDivider:!1,content:jx({allowedTenderOptions:r,canAddPaymentSources:n,deletePending:t,copy:e,showTransferAction:i})})}shouldRenderTransferScreen(){const e=this.interactionResponse;return(this.transferFlowActive||Sj(e))&&jj(e?.status)}renderTransferScreen(){const e=bj(this.interactionResponse),t="transfer"===this.pendingAction,r=this.transferFlowActive?vx({label:"Back",action:"back-to-payment-methods",disabled:t}):void 0;if(e.length<2)return this.renderWidgetShell({title:"Transfer funds",headerAction:r,showClose:!1,showDivider:!1,content:Ry`
          <div class="payment-flow">
            ${cx({title:"Transfer unavailable",message:"At least two verified bank or native accounts are required to transfer funds."})}
          </div>
        `});const n=[{value:"",label:"Select account"},...e.map(e=>({value:pj(e),label:dj(e)}))],i=qk(this.transferDraft),o=e.find(e=>pj(e)===i.fromSourceId),a=e.find(e=>pj(e)===i.toSourceId);return"confirm"===this.transferFlowScreen&&o&&a&&Lk(i,e)?this.renderTransferConfirmationScreen(o,a,i,t):this.renderWidgetShell({title:"Transfer funds",headerAction:r,showClose:!1,showDivider:!1,busy:t,overlay:t?Vj({title:"Initiating transfer",helper:"Submitting your transfer, please wait."}):void 0,content:Ry`
      ...
      `})}renderTransferConfirmationScreen(e,t,r,n){return this.renderWidgetShell({title:"Confirm transfer",headerAction:vx({label:"Back",action:"edit-transfer",disabled:n}),showClose:!1,showDivider:!1,busy:n,overlay:n?Vj({title:"Initiating transfer",helper:"Submitting your transfer, please wait."}):void 0,content:Ry`
        <div class="payment-flow card-form-screen">
          ${this.transferSubmitError?sx({variant:"danger",message:this.transferSubmitError}):""}
          ${fx([{label:"From",value:dj(e)},{label:"To",value:dj(t)},{label:"Amount",value:Vk(r)}])}
          <div class="primitive-footer card-form-primary">
            <div class="card-form-primary">
      ...
      `})}renderBankOutcomeScreen(){const e=Hj(this.bankOutcomeSource),t="declined"===e.kind||"pending"===e.kind;return this.renderWidgetShell({title:e.title,headerAction:t?vx({label:"Done",action:"done-bank-outcome"}):void 0,showClose:!1,showDivider:!1,content:Ry`
        <div class="verification-outcome-flow">
          ${sx({variant:e.variant,message:e.message})}
          ${this.bankOutcomeSource?nE(this.bankOutcomeSource):""}
          ${t?"":Ry`
                <div class="primitive-footer">
...
`,Uj=e=>"verify-payment-source"===e?Vj({title:"Verifying payment method",helper:"Verifying your payment method, please wait."}):"submit-required-info"===e?Vj({title:"Verifying account",helper:"Reviewing your verification information, please wait."}):void 0,Fj=e=>"verify-payment-source"===e?Uj(e):"delete-payment-source"===e?Vj({title:"Deleting payment method",helper:"Deleting your payment method, please wait."}):"attach-payment-source"===e?Vj({title:"Saving payment method",helper:"Saving your payment method, please wait."}):void 0,Hj=e=>{const t=e?.verificationInfo?.status;return"VERIFIED"===t?{kind:"verified",variant:"success",title:"Bank account verified",message:"Your bank account is connected and ready to use."}:"UNDER_REVIEW"===t?{kind:"pending",variant:"warning",title:"Bank account pending review",message:"We will update this bank account after review is complete."}:"REQUIRE_INFO"===t?{kind:"requires-info",variant:"warning",title:"Complete account verification",message:"Additional information is required before this bank account can be verified."}:"DECLINED"===t?{kind:"declined",variant:"danger",title:"Bank account verification failed",message:"This bank account could not be verified."}:{kind:"failed",variant:"danger",title:"Bank account not verified",message:"This bank account is not verified yet."}},Wj=(e,t=!1)=>{const r=oE(e),n=r.toLowerCase(),i=e?.verificationInfo?.status;return t?{kind:"failed",variant:"danger",title:"Bank account"===r?"Bank account verification failed":`${r} not verified`,message:`We could not verify this ${n} right now.`}:"VERIFIED"===i?{kind:"verified",variant:"success",title:"Bank account"===r?"Bank account verified":`${r} saved`,message:`This ${n} is verified and ready to use.`}:"UNDER_REVIEW"===i?{kind:"pending",variant:"warning",title:"Bank account"===r?"Bank account pending review":"Verification pending review",message:`We will update this ${n} after review is complete.`}:"REQUIRE_INFO"===i?{kind:"requires-info",variant:"warning",title:"Complete account verification",message:`Additional information is required before this ${n} can be verified.`}:"DECLINED"===i?{kind:"declined",variant:"danger",title:"Bank account"===r?"Bank account verification failed":`${r} declined`,message:`This ${n} could not be verified.`}:{kind:"failed",variant:"danger",title:"Bank account"===r?"Bank account not verified":`${r} not verified`,message:`This ${n} is not verified yet.`}},Kj=e=>{const t=e?.verificationInfo?.docs??[];return t.length>0?t:[{}]},Gj=(e,t=0)=>`${e?.docType??"DOCUMENT"}-${t}`,Jj=e=>"DRIVER_LICENSE_NUMBER"===e.docType||"SOCIAL_SECURITY_NUMBER"===e.docType,Yj=e=>!1!==e.mandatory,Zj=e=>Yj(e)?"required":"optional",Qj=e=>Ry`
  <span
    class="required-info-requirement-badge required-info-requirement-badge-${Zj(e)}"
  >
    ${(e=>Yj(e)?"Required":"Optional")(e)}
  </span>
`,Xj=e=>{const t=e.findIndex(e=>!Jj(e));return t>=0?Gj(e[t],t):void 0},eE=({doc:e,key:t,value:r})=>{if("SOCIAL_SECURITY_NUMBER"===e.docType)return tE({doc:e,key:t,value:r});const n=`required-info-${t}`;return Ry`
    <div
  ...
  `},tE=({doc:e,key:t,value:r})=>{const n=rE(e),i=(e=>{const t=Rk(e);return[t.slice(0,3),t.slice(3,5),t.slice(5,9)]})(r),o=`required-info-${t}-ssn-0`;return Ry`
    <div
  ...
  `},rE=e=>"BANK_STATEMENT"===e.docType?"Bank statement":"BANK_LETTER"===e.docType?"Bank letter":"BANK_CHECK"===e.docType?"Bank check":"DRIVER_LICENSE_PHOTO"===e.docType?"Driver license photo":"DRIVER_LICENSE_NUMBER"===e.docType?"Driver license number":"SOCIAL_SECURITY_NUMBER"===e.docType?"Social Security number":"Required document",nE=e=>{const t=(e=>({type:vj(e),brand:hj(e)}))(e),r=Wk(t),n=(e=>{if(!sE(e))return;const t=e.card?.expirationMonth?.trim(),r=e.card?.expirationYear?.trim();if(!t||!r)return;return`${t.padStart(2,"0")}/${r.slice(-2)}`})(e);return Ry`
    <div
      ...
      <div class="required-info-card-top">
        <div class="required-info-card-heading">
          <span class="required-info-card-label"
            >${(e=>aE(e)?"Bank account":sE(e)?"Card":cE(e)?"Native account":"Dynamic session card")(e)}</span
          >
          <span
            class="required-info-card-field required-info-card-name"
            data-required-info-card-field="name"
            >${(e=>sE(e)?e.card?.nameOnCard?.trim()||e.name?.trim()||lj(e):aE(e)?e.name?.trim()||e.bankAccount?.bankName?.trim()||lj(e):cE(e)?e.account?.nickName?.trim()||e.name?.trim()||lj(e):e.name?.trim()||lj(e))(e)}</span
          >
        </div>
      ...
      <div class="required-info-card-bottom">
        <div class="required-info-card-detail">
          ...
          <span class="required-info-card-status">
            ${ex(Rj(e.verificationInfo?.status))}
          </span>
        </div>
        ${n?Ry`
              <div class="required-info-card-detail">
  ...
  `},iE=e=>"REQUIRE_INFO"===e.verificationInfo?.status,oE=e=>e&&aE(e)?"Bank account":e&&cE(e)?"Native account":"Payment method",aE=e=>"BankAccount"===e.tenderType||"BANK_ACCOUNT"===e.tenderType,sE=e=>"Card"===e.tenderType||"CARD"===e.tenderType,cE=e=>"NativeAccount"===e.tenderType||"NATIVE_ACCOUNT"===e.tenderType;let uE,dE;const lE=async e=>{if(!window.PaymentSDK?.init)if(uE&&dE===e)await uE;else if(dE=e,uE=new Promise((t,r)=>{const n=document.querySelector(`script[data-commerce-hub-pay-by-bank-sdk="true"][src="${e}"]`),i=n??document.createElement("script"),o=()=>{i.removeEventListener("load",a),i.removeEventListener("error",s)},a=()=>{o(),t()},s=()=>{o(),uE=void 0,dE=void 0,r(new Error(`Unable to load Connect your bank script: ${e}`))};i.addEventListener("load",a,{once:!0}),i.addEventListener("error",s,{once:!0}),n||(i.src=e,i.async=!0,i.dataset.commerceHubPayByBankSdk="true",document.head.appendChild(i))}),await uE,!window.PaymentSDK?.init)throw new Error(`Connect your bank script loaded but window.PaymentSDK.init is unavailable: ${e}`)},hE=()=>{try{window.PaymentSDK?.destroy?.()}catch{}};export{bw as COMMERCE_HUB_BASE_URLS,Bx as CommerceHubConnect,vw as DEFAULT_COMMERCE_HUB_BASE_URL,Ax as ON_ADD_PAYMENT_METHOD_EVENT,Ox as ON_DELETE_PAYMENT_METHOD_EVENT,Nx as ON_ERROR_EVENT,Ix as ON_LIST_PAYMENT_METHODS_EVENT,Sx as ON_SELECT_EVENT,_x as ON_TRANSFER_EVENT,Cx as ON_VERIFY_PAYMENT_METHOD_EVENT,yw as createCommerceHubClient,$w as createCommerceHubConfigClient,ww as createCommerceHubMerchantClient,gw as resolveCommerceHubBaseURL};
